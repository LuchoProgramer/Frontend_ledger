...
export default function ProductForm({ initialData, onSuccess }: ProductFormProps) {
    ...
    const [formData, setFormData] = useState<ProductoFormData>({
        ...
        abv: initialData?.abv || '',
        google_category_id: initialData?.google_category_id || '',
    });
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Google Category ID Override */}
                <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Google Category ID (Override)</label>
                    <input
                        type="text"
                        ...
                        onChange={handleChange}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500"
                        placeholder="Ej: 499 (Opcional - Sobrescribe la categoría)"
                    />
                    <p className="mt-1 text-xs text-gray-500">
                        Si se deja vacío, se usará el ID de la categoría seleccionada.
                        <a
