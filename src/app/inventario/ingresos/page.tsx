...
import { Search, Trash2, PackagePlus, CheckCircle2, AlertCircle, Plus } from 'lucide-react';
...
const MOTIVO_CHIPS = ['Compra', 'Producción propia', 'Compra informal', 'Donación', 'Muestra'];
...
export default function IngresosInventarioPage() {
  ...
  function handleAgregarLinea() {
    ...
    if (!motivoNuevo.trim()) { setErrorLinea('El motivo es obligatorio.'); return; }
  ...
  async function handleConfirmar() {
    ...
    try {
      ...
      alert(e.message || 'Error al procesar el ingreso');
    } finally {
      setSubmitting(false);
    }
  }
        ...
        <div className="p-4 md:p-6 max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl shadow p-6">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-gray-900">Ingreso completado</h2>
            ...
            {resultado.exitosos.length > 0 && (
              <div className="mb-4">
                <div className="text-xs font-bold text-gray-500 uppercase mb-2">Ingresados</div>
                <div className="space-y-2">
                  {resultado.exitosos.map((e, i) => (
                    <div key={i} className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                      <span className="text-sm font-medium text-green-800">{e.nombre}</span>
                      <span className="text-sm text-green-700 font-bold">+{e.cantidad}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div className="p-4 md:p-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-gray-500 text-sm">Agrega productos a la lista y confirma todo en un solo ingreso.</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow p-5">
            ...
            <div className="relative mb-4">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-400"
              />
            ...
            {productoSeleccionado && (
              <div className="space-y-3">
                <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                  <div className="font-semibold text-green-800 text-sm">{productoSeleccionado.nombre}</div>
                  <div className="text-xs text-green-600">{productoSeleccionado.codigo_producto}</div>
                </div>
                ...
                {/* Cantidad */}
                <div>
                  <label className="block text-xs font-bold text-gray-600 mb-1">Cantidad a ingresar</label>
                  <input
                    type="number"
                    min="0.01"
                    step="0.01"
          ...
          <div className="bg-white rounded-2xl shadow p-5">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-base font-bold text-gray-800">
                Lista de ingreso
                {lineas.length > 0 && <span className="ml-2 text-green-600">({lineas.length})</span>}
              </h2>
            </div>

            {lineas.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-12 text-gray-400">
                <PackagePlus className="w-10 h-10 mb-2 opacity-30" />
                <p className="text-sm">Aún no hay productos en la lista</p>
              </div>
            ) : (
              <>
                {/* Desktop table */}
                  ...
                  <table className="min-w-full text-sm">
                    <thead>
                      <tr className="text-xs font-bold text-gray-500 uppercase border-b">
                        ...
                        <th className="pb-2 text-left pl-3">Motivo</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {lineas.map(l => (
                        <tr key={l.key}>
                          <td className="py-3 font-medium text-gray-800 max-w-[160px] truncate">{l.producto.nombre}</td>
                          <td className="py-3 text-gray-500 text-xs">{l.sucursal_nombre}</td>
                          <td className="py-3 text-right font-bold text-green-700">+{l.cantidad}</td>
                          <td className="py-3 text-gray-500 text-xs pl-3 max-w-[120px] truncate">{l.motivo}</td>
                  ...
                  {lineas.map(l => (
                    <div key={l.key} className="flex justify-between items-start p-3 bg-gray-50 rounded-lg">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-gray-500">{l.sucursal_nombre} · {l.motivo}</div>
                      </div>
                      <div className="flex items-center gap-3 ml-3">
                        <span className="font-bold text-green-700 text-sm">+{l.cantidad}</span>
                        <button onClick={() => handleEliminarLinea(l.key)} className="text-red-400 hover:text-red-600">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                ...
                <button
                  ...
                  disabled={submitting}
                  className="mt-5 w-full py-3 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 disabled:opacity-50 text-sm"
                >
                  {submitting ? 'Procesando...' : `Confirmar ingreso de ${lineas.length} producto${lineas.length !== 1 ? 's' : ''}`}
                </button>
              </>
            )}
          </div>
