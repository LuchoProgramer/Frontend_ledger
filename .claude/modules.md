...
| Captura de errores no manejados → backend | `src/lib/reportError.ts`, `src/app/global-error.tsx` | **DESPLEGADO 2026-07-21 (Version `322d1277`, confirmado en prod 2026-07-27):** ante un error no-chunk (fallback amable, no auto-reload), `global-error.tsx` llama a `reportClientError` — POST best-effort a `${apiUrl}/errores-frontend/` (`X-Tenant` si no es `public`), nunca lanza ni bloquea la UI. Backend: `core.api_publico.ClientErrorReportView` (`POST /api/errores-frontend/`, throttle `client_error_report` 20/min) persiste en `empresas.ClientErrorLog` para diagnóstico manual — sin dashboard de lectura aún. Verificado en prod: `POST https://api.ledgerxpertz.com/api/errores-frontend/` → 201; chunk `global-error-*.js` desplegado contiene la referencia al endpoint. Tests: `src/__tests__/reportError.test.ts`. |
