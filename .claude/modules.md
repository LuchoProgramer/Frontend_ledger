...
| Auto-reload version skew + captura de errores | `src/app/global-error.tsx`, `src/lib/chunkError.ts`, `src/lib/reportError.ts` | **2026-06-27 / 2026-07-20:** error boundary raíz. Auto-recarga ante `ChunkLoadError` (guard 10s via `sessionStorage`). Para cualquier otro error: (1) mensaje técnico visible en pantalla (`error.message`/`digest` truncados), y (2) POST best-effort (`reportClientError`) al backend `POST /api/errores-frontend/` con `X-Tenant` header si no es `public`. Fire-and-forget, nunca bloquea la UI. Tests: `src/__tests__/{chunkError,globalError,reportError}.test.ts(x)`. Instrumentación para diagnosticar crash en tablets de la_huequita. |
