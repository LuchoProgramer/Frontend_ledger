# Ajuste por Lote — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Agregar la página "Ajuste por Lote" (`/inventario/ajustes/lote`) que permite ajustar el inventario de múltiples productos en un solo envío, con motivo global y selector ENTRADA/SALIDA por línea.

**Architecture:** Página nueva en `src/app/inventario/ajustes/lote/page.tsx` siguiendo el patrón de `ingresos/page.tsx` (dos paneles, bulk API). Se modifica `DashboardLayout.tsx` para agregar el ítem de menú y `ajustes/page.tsx` para agregar un botón de acceso rápido.

**Tech Stack:** Next.js 15 App Router, React, TailwindCSS, lucide-react, `api.ajusteInventarioBulk()` (endpoint ya existe en backend)

**Spec:** `docs/superpowers/specs/2026-03-24-ajuste-por-lote-design.md`
...
|--------|---------|------------|
| Crear | `src/app/inventario/ajustes/lote/page.tsx` | Página completa nueva |
| Modificar | `src/components/DashboardLayout.tsx` | Import `Layers` + nuevo ítem en sección Inventario |
| Modificar | `src/app/inventario/ajustes/page.tsx` | Botón de acceso rápido en el header |

---

## Task 1: Agregar ítem de menú en DashboardLayout

**Files:**
- Modify: `src/components/DashboardLayout.tsx:9-35` (imports) y `:144-153` (sección Inventario)

- [ ] **Step 1: Agregar import de `Layers`**

En `src/components/DashboardLayout.tsx`, dentro del bloque de imports de lucide-react (línea ~34, después de `Gift`), agregar `Layers`:
...
- [ ] **Step 2: Agregar ítem al menú de Inventario**

En la sección `Inventario` del array `menuItems` (línea ~149, después del ítem "Ajuste de Stock"), insertar:
...
Expected: sin errores de TypeScript en `DashboardLayout.tsx`

- [ ] **Step 4: Commit**

```bash
git add src/components/DashboardLayout.tsx
git commit -m "feat: add Ajuste por Lote menu item to sidebar"
```

---

## Task 2: Agregar botón de acceso rápido en ajustes/page.tsx

**Files:**
- Modify: `src/app/inventario/ajustes/page.tsx:244-254` (bloque header de la página)

- [ ] **Step 1: Agregar import de `Link` y `Layers`**

En `src/app/inventario/ajustes/page.tsx`, agregar `Layers` al bloque de imports de lucide-react existente y agregar import de `Link` de next/link:

```typescript
import Link from 'next/link';
```

Y en el bloque de lucide-react agregar `Layers`:
```typescript
import {
  Search,
  ChevronRight,
...
- [ ] **Step 2: Agregar botón en el header de la página**

Dentro del bloque `{/* Page header */}` (línea ~246), después del párrafo de descripción (`<p className="text-sm text-gray-500 ml-9">`), agregar el botón:
...
```bash
git add src/app/inventario/ajustes/page.tsx
git commit -m "feat: add quick access button to Ajuste por Lote from ajustes page"
```

---

## Task 3: Crear página Ajuste por Lote

**Files:**
- Create: `src/app/inventario/ajustes/lote/page.tsx`

- [ ] **Step 1: Crear directorio y archivo**

Crear `src/app/inventario/ajustes/lote/page.tsx` con el siguiente contenido completo:
...
Expected: sin errores de TypeScript en `ajustes/lote/page.tsx`

- [ ] **Step 3: Verificar que la ruta existe correctamente**

```bash
ls src/app/inventario/ajustes/lote/
```
...
```bash
git add src/app/inventario/ajustes/lote/page.tsx
git commit -m "feat: add Ajuste por Lote page with bulk inventory adjustment"
```
...
- [ ] Navegar a `/inventario/ajustes` — debe aparecer el botón "Ajuste por Lote" en el header
- [ ] El menú lateral debe mostrar "Ajuste por Lote" bajo la sección Inventario con ícono de capas
- [ ] Navegar a `/inventario/ajustes/lote` — debe cargar la página con los dos paneles
- [ ] Agregar 2+ productos con tipos distintos (ENTRADA/SALIDA) y confirmar con motivo
- [ ] La pantalla de resultado debe mostrarse correctamente
- [ ] El botón "Nuevo lote" resetea el formulario; "Ir a inventario" navega a `/inventario`
