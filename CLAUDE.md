...
### ✅ Destrancar cola offline del POS (⚡ atascado bloqueaba cerrar caja) — DESPLEGADO 2026-07-16 (Version ID `5e33bea3`)

**Incidente (la_huequita, caja de Emili):** una venta quedó `PENDIENTE` en Dexie por un blip y el ⚡ 1 bloqueó el botón de cerrar caja indefinidamente — la cola solo se re-sincronizaba con `online`/`visibilitychange`, eventos que una tablet kiosk que nunca sale del POS jamás dispara (el servidor estaba sano: 100% de POSTs 201 en 72h). **Fix (commit `cb35f3b`):** (1) `useOfflineQueue` drena la cola **al montar** si hay conexión, (2) `usePOSPayment.processSale` la drena **tras cada venta online exitosa**, (3) el badge ⚡ de `POSLayout` ahora es un **botón** "Sincronizar ventas pendientes" (`onSyncPending`). TDD: 6 tests nuevos en 3 archivos, suite 117/117, tsc limpio. **Deploy:** persepolis SIN turno (gate OK); la_huequita con turno 91 abierto (el bloqueado) pero sin ventas hacía ~2h — subió solo 3 assets (BUILD_ID + chunk POS `page-4a19705a21a892a2.js` + 1 compartido, 111 sin cambios → precaching mínimo). Smoke: la_huequita `/`,`/pos`,`/sw.js` → 200, chunk contiene "Sincronizar ventas pendientes"; persepolis `/pos` → 200. **Operativa post-deploy:** la tablet atascada se destranca al aceptar el banner "Nueva versión" (reload → sync on-mount) o, con el código viejo, minimizando y volviendo al navegador. Sin riesgo de duplicados (`venta_uuid` idempotente).
...
### ✅ Validación del transportista en guías — DESPLEGADO 2026-06-17 (Version ID `9de9f5c3`)

`src/lib/transportista.ts::validarTransportista` (espejo del backend `facturacion/utils/transportista.py`, Tabla 6 SRI v2.32) + `src/app/guias/nueva/page.tsx`: el form de traslado arranca con default `ruc='9999999999999'` (consumidor final) + `razon_social='CONSUMIDOR FINAL'`, y al continuar valida (rechaza basura como `002`, normaliza 10→13 nueves). Raíz de las 2 guías NAT de la_huequita (transportista marcado como RUC con longitud inválida). Backend ya desplegado (blindaje server-side). Commits: `151535e` (helper TS), `6797c02` (form). **Deploy con persepolis SIN turno (gate OK)**; la_huequita estaba vendiendo pero el deploy subió solo 2 assets (BUILD_ID + chunk del form de guías `page-ef3d75e221907a02.js`, NO el chunk del POS de ventas → cero impacto en ventas en curso). Smoke: `/`,`/pos`,`/guias/nueva`,`sw.js` → 200; persepolis `/pos` → 200; el chunk del form contiene el default `9999999999999`. **Confirmado el mismo día:** el fix del toggle (abajo) quedó validado en prod — la_huequita emitió 18 facturas ELECTRÓNICAS AUTORIZADAS el 2026-06-17 (ids 382-399, `001-001-000010892`+), cero internas, tras recargar el POS.
